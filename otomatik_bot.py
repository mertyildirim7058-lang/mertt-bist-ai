...
def analyze_hybrid(ticker, history_data):
    ...
    try:
        ...
        # B. ÖN ELEME (Hız İçin)
        # RSI 70 üstündeyse (Pahalı) veya 30 altındaysa (Ucuz) ilgilen, yoksa geç
        ...
        if live_price and live_price > 0:
            ...
            # %20'den az fark varsa güven ve kullan
            if abs(live_price - last_yahoo) / last_yahoo < 0.20:
                df.iloc[-1, df.columns.get_loc('Close')] = live_price
        
        # D. TEKNİK ANALİZ (Gelişmiş)
        ...
        # F. HABER ANALİZİ (Sadece Yüksek Puanlılara)
        if score >= 70:
...
def main():
    ...
    # 30'lu Paketler (Yahoo Bloklamasın Diye)
    chunk_size = 30
