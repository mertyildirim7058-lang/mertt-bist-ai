...
def get_bist_full_list():
    """Eğer siteye ulaşılamazsa kullanılacak GÜNCEL TAM LİSTE (Semboller)"""
    return [
    ...
    return sorted(list(set(yedek_liste)))

@st.cache_data(ttl=600)
def tum_hisseleri_getir():
    """Önce İş Yatırım'dan canlı çeker, çekemezse TAM LİSTE döner"""
    canli_liste = []
    try:
    ...
    # Eğer canlı çekim başarılıysa onu dön, yoksa Full Listeyi dön
    if len(canli_liste) > 50:
        return sorted(list(set(canli_liste)))
    else:
        return get_bist_full_list()
    ...
    if menu == "💬 Hisse Sor":
        ...
        if btn and sembol:
            with st.spinner(f"{sembol} analiz ediliyor..."):
                ...
                else: st.error("Hisse bulunamadı veya verisi yetersiz.")
        ...
        # LİSTE KONTROLÜ
        if not tum_hisseler:
            st.error("⚠️ HATA: Piyasa verilerine şu an ulaşılamıyor.")
            st.stop()
            
        st.info(f"Takipteki Hisse Sayısı: {len(tum_hisseler)}")
        tarama_limiti = st.slider("Aynı anda kaç hisse taransın?", 10, 50, 20)
