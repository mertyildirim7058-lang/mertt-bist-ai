...
# 3. GELİŞMİŞ HABER MOTORU (DÜZELTİLDİ)
class GlobalIntel:
    def __init__(self):
        ...
        # Google News engelini aşmak için header
    ...
    def analyze_news(self, query_type="GENEL", ticker=""):
        ...
        # Sorguları çeşitlendir (Daha çok sonuç bulmak için)
        if query_type == "HISSE":
            feeds = [
                f"https://news.google.com/rss/search?q={ticker}+hisse&hl=tr&gl=TR&ceid=TR:tr",
                f"https://news.google.com/rss/search?q={ticker}+kap&hl=tr&gl=TR&ceid=TR:tr"
            ]
        else:
            ...
            try:
                ...
                if r.status_code == 200:
                    ...
                    for entry in feed.entries[:10]: 
                        ...
                        # Tarih Kontrolü (7 GÜNE ÇIKARILDI)
                        try:
                            if hasattr(entry, 'published_parsed'):
                                ...
                                if days_diff <= 7: # Son 1 haftanın haberlerini al
                                    ...
                                    news_display.append({"Title": title, "Link": link, "Date": date_str})
                        except: pass
            except: pass
        ...
        if btn and sembol:
            with st.spinner(f"{sembol} için formasyonlar, indikatörler ve haberler taranıyor..."):
                ...
                if res:
                    ...
                    st.markdown("---")
                    st.subheader(f"📰 {sembol} ile İlgili Son Gelişmeler")
                    if res['Haberler']:
                        for n in res['Haberler']:
                            st.markdown(f"🔹 **[{n['Title']}]({n['Link']})** - *{n['Date']}*")
                    else: st.info("Son 1 hafta içinde kritik bir haber bulunamadı.")
