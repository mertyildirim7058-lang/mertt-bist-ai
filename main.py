...
# --- 3. YEDEK LİSTE (HAYAT KURTARICI) ---
def get_backup_list():
    """Siteye ulaşılamazsa kullanılacak GÜNCEL TAM LİSTE"""
    return [
...
# --- ASLA ÇÖKMEYEN LİSTE ÇEKİCİ ---
@st.cache_data(ttl=600)
def get_live_tickers():
    canli_liste = []
    
    # 1. İş Yatırım'ı Dene (Canlı)
    try:
    ...
    # 2. Karar Anı: Canlı mı, Yedek mi?
    if len(canli_liste) > 50: 
        return sorted(list(set(canli_liste)))
    else: 
        # Eğer siteye ulaşamazsa, yedek listeyi devreye sok
...
class GlobalIntel:
    ...
    def analyze_news(self, query_type="GENEL", ticker=""):
        ...
        feeds = []
        if query_type == "HISSE":
            feeds = [f"https://news.google.com/rss/search?q={ticker}+hisse+kap+haberleri&hl=tr&gl=TR&ceid=TR:tr",
